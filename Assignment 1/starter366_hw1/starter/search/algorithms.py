...
class Dijkstra(Search):
    ...
    def search(self, start, goal):
        ...
        self.OPEN.clear()
        self.CLOSED.clear()
        node_expanded = 0  # Number of nodes expanded counter
        ...
        while (len(self.OPEN) != 0):
            ...
            for k in self.map.successors(n):  # k is the child of n
                delta_x = n.get_x() - k.get_x()
                delta_y = n.get_y() - k.get_y()

                if k.state_hash() not in self.CLOSED:
                    k.set_cost(n.get_cost() + self.map.cost(delta_x, delta_y))  # g(k) <- g(n) + C(n,k)
                    heapq.heappush(self.OPEN, k)
                    self.CLOSED[k.state_hash()] = k

                else:  # If k.state_hash() in self.CLOSED:
                    if (n.get_cost() + self.map.cost(delta_x, delta_y)) < self.CLOSED[k.state_hash()].get_cost():
                        self.CLOSED[k.state_hash()].set_cost(n.get_cost() + self.map.cost(delta_x, delta_y))
    ...
    def search(self, start, goal):
        ...
        while (len(self.OPEN) != 0):
            ...
            for k in self.map.successors(n):  # k is the child of n
                ...
                if k.state_hash() not in self.CLOSED:
                    k.set_g(n.get_g() + self.map.cost(delta_x, delta_y))  # g(k) <- g(n) + C(n,k)
                    k.set_h(self.h_value(k, goal))
                    k.set_cost(k.get_g() + k.get_h())
                    heapq.heappush(self.OPEN, k)
                    self.CLOSED[k.state_hash()] = k

                else:  # If k.state_hash() in self.CLOSED:
                    if (n.get_g() + self.map.cost(delta_x, delta_y)) < self.CLOSED[k.state_hash()].get_g():
                        temp = self.CLOSED[k.state_hash()]
                        temp.set_g(n.get_g() + self.map.cost(delta_x, delta_y))
                        temp.set_cost(temp.get_g() + temp.get_h())
                        heapq.heapify(self.OPEN)
